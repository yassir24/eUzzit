...
@Component({
  selector: 'app-select-product-cable',
  templateUrl: './select-product-cable.page.html',
  styleUrls: ['./select-product-cable.page.scss'],
})
export class SelectProductCablePage implements OnInit {
  ...
  constructor(
    ...
    //     this.data = this.router.getCurrentNavigation().extras.state.cable;
  ...
  async ngOnInit() {
    if (this.cabledata) {
      this.title = this.cabledata.name + " Recharge";
      if (this.cabledata.shortname !== "startimes") {
        ...
        this.listed = false;
        this.subtitle = "Startimes TV Recharge";
    ...
    } else if (this.cableid) {
      this.billerService.getCableProducts(this.cableid).subscribe((res) => {
        console.log(res.data.products);
        console.log(res.data.service);
        this.products = res.data.products;
        this.service = res.data.service;
        this.service_id = this.cableid;
        // for (const iterator of data.wallets) {
        //   if (iterator.slug !== 'shoppy') {
        //     this.wallets.push(iterator);
        //   }
    ...
    this.walletService.getMyWallets().then((wallet) => {
      for (var i = 0; i < wallet.length; i++) {
        wallet[i].balance = Number(wallet[i].balance).toFixed(2);
      }
      console.log(wallet);
  ...
  async setCableProduct(product) {
    await this.events.publish('setcableproduct', {product: product});
  }
